...
**Rewards**: $R(s,a)$ is the random reward function when the agent takes action $a$ in the state $s$. $r(s,a) = \mathbb E [R(s,a)]$ is the expected reward function which is only dependent on the corresponding state $s$ and action $a$.
...
**Partial Order Relation**: Given two policy $\pi_1$ and $\pi_2$, we have $\pi_1 > \pi_2$, if and only if for all states $s$, $V_{\pi_1} > V_{\pi_2}$ holds. (Other partial order symbols like $<$ and $\leq$ follow similar rules)

**Optimal Policy**: A policy $\pi_\star$ is optimal if and only if $\pi_\star \geq \pi$ for all states $\pi$.

# Prerequisite
## Bellman Optimality Equation
We are going to probe into the existence of optimal policy. So before diving into the formal proof of this theorem, we first explore some useful properties of optimal policy.
...
Employ this formula iteratively, a series of inequalities are derived
$$\begin{align*}
...
In this process, the partial order in (1) cannot always be equal signs if $s^\prime$ is a suboptimal state. Therefore, $V_{\pi_\star} \leq V_{\hat\pi}$ and for suboptimal states $s$, we conclude that $V_{\pi_\star}(s) < V_{\hat\pi}(s)$ which meaning that the policy $\pi_\star$ is not an optimal policy, leading to a contradiction.
...
**Remark**: *Corollary II* proves a necessary condition of an optimal policy.

## Unique Solution
In this section, we will prove that there is one and only one solution of the *Bellman Optimality Equation*.
...
We have proved that all optimal policies must satisfy *Bellman Optimality Equation* and that *Bellman Optimality Equation* must have a unique solution $\hat V_\star$. In the last step, we are will show that the policy correspond to $\hat V_\star$ is actually an optimal policy.

The transformation $T$ we use in the proof of *Lemma II* is quite important in Markov Decision Process. So let us re-emphasize the definition of $T$ and bring out a little lemma from it.
...
**Remark**: This lemma is just rewriting what we did in *Proposition I*. Do you remember how we prove this lemma there? Check the **Definition** section if you forgot what the $\geq$ symbol mean.
...
According to *Lemma II*, we know that $V$ is actually the unique solution $\hat V_\star$. If there exists a state-value function $ \tilde V$ s.t. $\tilde V(\tilde s) > V(\tilde s)$ at some state $\tilde s$. Using the transformation $T$, we get $\hat V = \lim _{n \rightarrow \infty} T^{n}(\tilde V) $. According to *Lemma II*, $\hat V$ is exactly $\hat V _\star$ as well. Through the repeated use of *Lemma III*, a inequality can be obtained
$$
\begin{align*}
