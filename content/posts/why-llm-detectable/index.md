...
The pretraining dataset is scrawled from the Internet. Formal, accurate text is constructed in a strongly constrained way because you have to obey all the grammar, standard wording devoid of emotion. This consistency enables it to be ragarded as being in a similar context, at least in the stylish dimension. On the other hand, in a chatroom, people speak in their own unique ways, making informal context sparse and long-tailed. We claim that this imbalance can cause performance difference on a text-generation model.

Why would we say the data distribution is long-tailed? If it is not, imagine the following situation: our custumized dataset consists only of wiki-style text and chatroom-style text. There are 1M tokens in the style of wiki and there are 10 chatroom styles, each with 1k tokens. Then only ~1% data is in chatroom styles. So even our model have a poor performance on the chatroom data, we still have a 99% chance of not being able to distinguish human's text and AI's text, as long as the model fits well on wiki. But the reality is not the case. The discrimination is visible over a large amount of fields.

Additionally, this analysis can not only apply to the context but also to intermediate levels. This leads us to the hierarchical part of the hypothesis. Suppose human generate the next token based on a hierarchy of context
